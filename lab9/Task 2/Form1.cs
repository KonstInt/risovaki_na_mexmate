...
namespace Task_3
{
    ...
    public class Polyhedron
    {
        ...
        public void updLightLocations(double light_location_x, double light_location_y, double light_location_z)
        {
            ...
            // Recompute normals and diffuse lighting for Phong shading
            updPoints();
            fillPointToPhongDiffuse();
        }

        public void fillPointToPhongDiffuse()
        {
            foreach (var indpoint in point_to_normal.Keys)
            {
                ...
                // Light vector from point to light source (light location minus point location)
                PointPol light_direction = new PointPol(light_loc_x - px, light_loc_y - py, light_loc_z - pz);
                light_direction = light_direction.normalize();

                // Normal vector at the point
                PointPol normal = point_to_normal[indpoint];

                // Diffuse lighting component (Lambertian reflection model)
                double diffuse = Math.Max(scalarProduct(normal, light_direction), 0.0);

                // You can add specular here if you want to simulate highlights
                // For example, Phong model uses a specular reflection based on the view direction and light direction
                // Specular component calculation (Phong model)
                // PointPol view_direction = new PointPol(view_x - px, view_y - py, view_z - pz); // Assuming view point is known
                // PointPol reflect_direction = normal * 2.0 * scalarProduct(light_direction, normal) - light_direction;
                // double specular = Math.Pow(Math.Max(scalarProduct(view_direction.normalize(), reflect_direction), 0.0), shininess);

                // Set the diffuse value for Phong shading
                point_to_diffuse[indpoint] = diffuse;
            }
        }
